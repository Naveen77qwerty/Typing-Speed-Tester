...
public class WordGenerator {
    ...
    private void initializeWords() {
        ...
        this.words = new ArrayList<>(Arrays.asList(commonWords)); 
        //creates a current instance of the list which is mutable by size and acessible by add, remove, etc.
    }
    
    public String getRandomSentence() {
        // Generate a random sentence length based on difficulty
        int sentenceLength;
        switch (currentDifficulty) {
            case EASY:
                sentenceLength = 5 + random.nextInt(3); // 5-7 words
                break;
            case HARD:
                sentenceLength = 10 + random.nextInt(9); // 10-20 words
                break;
            default: // MEDIUM
                sentenceLength = 7 + random.nextInt(6); // 7-12 words
                break;
        }
        ...
        for (int i = 0; i < sentenceLength; i++) {
            ...
            // Add comma after every 3rd word
            else if (i % 3 == 0 && i != 0) {
                sentence.append(", ");
            }
